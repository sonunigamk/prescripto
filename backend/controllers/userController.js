...
const registerUser = async (req, res) => {
    try {
        ...
        if (!name || !email || !password) {
            return res.status(400).json({ success: false, message: "Missing Details" }); // Use 400 for bad requests
        }
...
const updateProfile = async (req, res) => {
    try {
        ...
        const updatedUser = await userModel.findByIdAndUpdate(userId, updateData, { new: true }); //Added {new: true}
...
const cancelAppointment = async (req, res) => {
    try {
        ...
        if (!updatedAppointment) {
            return res.status(404).json({ success: false, message: "Appointment not found" }); // Handle the case where the update fails.
        }
